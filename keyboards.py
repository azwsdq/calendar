...
def create_calendar_keyboard(year: int, month: int) -> InlineKeyboardMarkup:
    ...
    builder.button(text=f"📅 {month_name}", callback_data="ignore")

    prev_month = month - 1 if month > 1 else 12
    prev_year = year if month > 1 else year - 1
    builder.button(text="◀️", callback_data=f"prev_{prev_year}_{prev_month}")

    next_month = month + 1 if month < 12 else 1
    next_year = year if month < 12 else year + 1
    builder.button(text="▶️", callback_data=f"next_{next_year}_{next_month}")
...
def create_event_keyboard(event_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✏️ Изменить", callback_data=f"edit_{event_id}")
    builder.button(text="🗑️ Удалить", callback_data=f"delete_{event_id}")
    builder.adjust(2)
    return builder.as_markup()
